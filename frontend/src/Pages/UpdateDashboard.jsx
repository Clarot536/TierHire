...
const InfoSection = ({ icon, title, items, handleAddItem, handleRemoveItem, handleEditItem, inputFields, currentItem, setCurrentItem }) => {
        ...
        {items.map((item) => (
          <div key={item.id} className="list-item">
            ...
            <div className="item-actions">
              <button onClick={() => handleEditItem(item)} className="icon-btn edit-btn" aria-label="Edit"><EditIcon /></button>
              <button onClick={() => handleRemoveItem(item.id)} className="icon-btn remove-btn" aria-label="Remove"><TrashIcon /></button>
            </div>
          </div>
        ))}
...
const UpdateDashboard = () => {
  ...
  // --- REFACTORED STATE: Matches the requested structure ---
  ...
  // Local states for the input fields of each section
  ...
  const handleDomainToggle = (domain) => {
    setFormData(prevData => {
      const currentlySelected = prevData.domains; // Updated to use 'domains' key
      const newSelected = currentlySelected.includes(domain)
        ? currentlySelected.filter((d) => d !== domain)
        : [...currentlySelected, domain];
      return { ...prevData, domains: newSelected };
    });
  };

  const handleCvChange = (event) => {
    if (event.target.files && event.target.files[0]) {
      // Store the entire File object. This is what you'll upload.
      setFormData(prevData => ({ ...prevData, cv: event.target.files[0] }));
    }
  };
  
  const createItemHandlers = (formKey, currentItem, setCurrentItem, resetItem) => ({
    handleAddItem: () => {
      ...
      setFormData(prevData => {
        ...
        return { ...prevData, [formKey]: updatedItems };
      });
    ...
    handleRemoveItem: (id) => {
      setFormData(prevData => {
        const updatedItems = prevData[formKey].filter(item => item.id !== id);
        return { ...prevData, [formKey]: updatedItems };
      });
    },
  ...
  const skillHandlers = createItemHandlers('skills', currentSkill, setCurrentSkill, { name: '' });
  const projectHandlers = createItemHandlers('projects', currentProject, setCurrentProject, { title: '', description: '' });
  const educationHandlers = createItemHandlers('education', currentEducation, setCurrentEducation, { degree: '', institution: '', year: '' });
  const experienceHandlers = createItemHandlers('experience', currentExperience, setCurrentExperience, { role: '', company: '', duration: '' });
  ...
  return (
    <div className={`dashboard-container ${theme}`}>
      ...
      <main className="dashboard-main">
        <div className="domains-section glassy-card">
          ...
          <div className="domains-grid">
            {domains.map((domain) => (
              <div
                key={domain}
                className={`domain-card glassy-card ${formData.domains.includes(domain) ? 'selected' : ''}`}
        ...
        <div className="profile-details">
          ...
          <div className="profile-section glassy-card">
              ...
              <label className="custom-file-upload">
                  <input type="file" onChange={handleCvChange} accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" />
                  <span>Choose File</span>
              </label>
              {formData.cv && <p className="file-name">Selected: {formData.cv.name}</p>}
          </div>
